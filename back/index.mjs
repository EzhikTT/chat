    ...
    if(req.method === 'OPTIONS') {
        ...
        try {
            if(await BaseController.checkAuthorize(req, res)) {
                ...
                else if(BaseController.get('/chats/:id/message', req, res)) {
                    await MessageController.getByChatId(req, res, req.params.id)
                }
                else if(await BaseController.post('/chats/:id/message', req, res)) {
                    await MessageController.save(req, res, {...req.body, chatId: req.params.id})
                }
                else if(BaseController.delete('/chats/:chatId/message/:id', req, res)) {
                    await MessageController.delete(req, res, req.params.id)
                }
                else if(await BaseController.put('/chats/:chatId/message/:id', req, res)) {
                    await MessageController.update(req, res, req.params.id, {...req.body, chatId: req.params.chatId})
                }
                else {
                    console.log('auth')
                    res.statusCode = 404
